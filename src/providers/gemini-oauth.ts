...
async function tryOpenBrowser(url: string): Promise<boolean> {
  ...
  for (const candidate of candidates) {
    try { await execFileAsync(candidate.command, candidate.args); return true; } catch {}
  }
  return false;
}
...
export class GeminiOAuthAdapter implements ProviderLoginAdapter {
  ...
  async start(method: 'oauth'): Promise<LoginStartResult> {
    ...
    try {
      ...
      throw new Error(`Unable to start Gemini OAuth callback server on port ${port}: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  private async waitForCallback(input: { code?: string; state?: string }): Promise<{ code: string; state: string }> {
    if (input.code && input.state) return input;
    if (this.callback) return this.callback;
    const started = Date.now();
    while (!this.callback) {
      if (Date.now() - started >= CALLBACK_TIMEOUT_MS) throw new Error('Gemini OAuth login timed out after 5 minutes. Run login again.');
