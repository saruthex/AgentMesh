...
export async function startLogin(provider: string, method: LoginAuthMethod = 'oauth'): Promise<LoginStartResult> {
  ...
  if (!adapter.methods.includes(method)) throw new Error(`Login method "${method}" is not supported for provider: ${provider}`);
...
export async function completeLogin(provider: string, input: { code?: string; state?: string }): Promise<LoginCompletionResult> {
  ...
  saveCredential({ provider: result.provider, accessToken: result.accessToken, refreshToken: result.refreshToken, expiresAt: result.expiresAt, accountLabel: result.accountLabel });
  return result;
}

export function loginStatus(provider?: string): string[] {
  const providers = provider ? [provider] : loginRegistry.list();
  return providers.map(name => `${name}: ${getCredential(name) ? 'authenticated' : 'not authenticated'}`);
}
