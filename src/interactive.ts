...
const COMMANDS = ['help', 'status', 'agents', 'providers', 'auth', 'connect', 'switch', 'chat', 'plan', 'swarm', 'history', 'login', 'exit', 'quit'];
...
function printAgents(root: string): void {
  ...
  if (!config.agents.length) {
    console.log(chalk.yellow('No agents connected yet. Use: connect <provider> [name]'));
    return;
  }
  for (const agent of config.agents) {
    const active = config.activeAgent === agent.id;
    const marker = active ? chalk.green('●') : chalk.gray('○');
    console.log(`${marker} ${chalk.bold(agent.name)}  ${chalk.gray(agent.provider)}  ${chalk.gray(agent.role ?? 'general')}${agent.model ? `  ${chalk.gray(agent.model)}` : ''}${active ? chalk.green('  active') : ''}`);
  }
}
...
async function runCommand(input: string, root: string, rl: Interface): Promise<boolean> {
  const parts = input.match(/(?:[^\s"]+|"[^"]*")+/g) ?? [];
  const [command, ...args] = parts.map(value => value.replace(/^"|"$/g, ''));
  ...
  switch (command) {
    ...
    default:
      console.log(chalk.yellow(`Unknown command: ${command}. Type [1mhelp[0m for available commands.`));
      return true;
  }
}
...
export async function startInteractiveMode(): Promise<void> {
  const root = requireProject();
  renderHeader(root);
  const rl = createInterface({ input: process.stdin, output: process.stdout, prompt: chalk.green('agentmesh> ') });
  let running = true;
  while (running) {
    const line = await new Promise<string>(resolve => rl.question(chalk.green('agentmesh> '), resolve));
    try {
      running = await runCommand(line.trim(), root, rl);
    } catch (error) {
      const text = error instanceof Error ? error.message : String(error);
      console.log(chalk.red(`✗ ${text}`));
    }
